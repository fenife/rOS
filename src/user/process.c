...
/* 构建用户进程初始上下文信息
 * 用户程序一般是从文件系统上加载到内存的，因此进程名便是进程的文件名
 */
void start_process(void * filename_)
{
    void * function = filename_;
    struct task_struct * cur = running_thread();
    /* 使指针self_kstack跨过线程线程，指向中断栈，
     * 保存用户进程的上下文环境
     */
    cur->self_kstack += sizeof(struct thread_stack);
    struct intr_stack * proc_stack =
                        (struct intr_stack *)cur->self_kstack;
    proc_stack->edi = 0;
    proc_stack->esi = 0;
    proc_stack->ebp = 0;
...
/* 创建用户进程的页目录表，将当前页表中表示内核空间的pde复制到新建的
 * 页目录表中，实现内核空间共享
...
uint32_t * create_page_dir(void)
{
    ...
    {
        printk("create_page_dir: get_kernel_pages failed!\n");
        return NULL;
    }

    /* 1.先复制属于内核空间的页目录项到此用户进程的页目录表中
    ...
    /* 2.更新页目录地址
     * 页目录地址存放在页目录的最后一项，更新页目录地址为新页目录的物理地址
     * 即把最后一个页目录项更新为用户进程自己的页目录表的物理地址
     */
...
void create_user_vaddr_bitmap(struct task_struct *user_prog)
{
    ...
    user_prog->user_vaddr.vm_start = USER_VADDR_START;
    user_prog->user_vaddr.bm.bits = get_kernel_pages(bitmap_pg_cnt);
    user_prog->user_vaddr.bm.len  =
                (0xc0000000 - USER_VADDR_START) / PG_SIZE / 8;
    bitmap_init(&user_prog->user_vaddr.bm);
}

/* 创建用户进程，并将其加入到就绪队列等待执行 */
void process_execute(void *filename, char *name)
{
    /* pcb是内核的数据结构，由内核来维护进程信息，因此要在内核内存池中申请 */
    ...
    kassert(!elem_find(&thread_all_list, &thread->all_list_tag));
