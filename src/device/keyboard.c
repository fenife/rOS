...
/* ext_scancode用于记录makecode是否以0xe0开头（扩展的扫描码） */
...
static void intr_keyboard_handler(void)
{
   ...
   uint16_t scancode = inb(KBD_BUF_PORT);
    ...
    /* 如果上次是以0xe0开头，和此次的扫描码合并成一个完整的扫描码 */
    if (ext_scancode)
    {
    ...
    break_code = ((scancode & 0x0080) != 0);    /* 标记此次为断码 */

    /* 若是断码break_code(按键弹起时产生的扫描码) */
    if (break_code)
    {
        /* 由于ctrl_r和alt_r的make_code和break_code都是两字节，
         * 所以可用下面的方法得到其make_code(按键按下时产生的扫描码)
