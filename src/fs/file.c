...
{
    int32_t bit_idx = bitmap_alloc(&part->inode_bm, 1);
    if (bit_idx == -1) 
    {
        return -1;
    }
    bitmap_set(&part->inode_bm, bit_idx, 1);
    return bit_idx;
}
...
int32_t block_bitmap_alloc(struct partition* part) {
    int32_t bit_idx = bitmap_alloc(&part->block_bm, 1);
    if (bit_idx == -1) 
    {
        return -1;
    }
    bitmap_set(&part->block_bm, bit_idx, 1);
    
    /* 和inode_bitmap_malloc不同，此处返回的不是位图索引，
     * 而是具体可用的扇区地址 
     */
...
void bitmap_sync(struct partition *part, uint32_t bit_idx, 
                        uint8_t btmp_type) 
{
    /* 本i结点索引相对于位图的以扇区为单位的偏移量 */
    uint32_t off_sec = bit_idx / 4096;     
    
    /* 本i结点索引相对于位图的以字节为单位的偏移量 */
    ...
    switch (btmp_type) 
    {
        case INODE_BITMAP:
            ...
            bitmap_off = part->inode_bm.bits + off_size;
            break;

        case BLOCK_BITMAP: 
            sec_lba = part->sb->block_bitmap_lba + off_sec;
            bitmap_off = part->block_bm.bits + off_size;
            break;
    }
...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag) 
{
     ...
     */
    struct inode* new_file_inode = (struct inode*)
                        sys_malloc(sizeof(struct inode)); 
...
int32_t file_write(struct file *file, const void *buf, uint32_t count) 
{
    ...
    kassert(file_will_use_blocks <= 140);
    ...
    { 
        ...
        { 
            ...
            kassert(file->fd_inode->i_sectors[12] != 0);
    ...
    else 
    {
        ...
        if (file_will_use_blocks <= 12 ) 
        {
            ...
            block_idx = file_has_used_blocks - 1;
            kassert(file->fd_inode->i_sectors[block_idx] != 0);
            all_blocks[block_idx] = file->fd_inode->i_sectors[block_idx];
            ...
            while (block_idx < file_will_use_blocks) 
            {
                ...
                if (block_lba == -1) 
                {
                    printk("file_write: block_bitmap_alloc for"
                                        " situation 1 failed\n");
                    return -1;
                }
...
int32_t file_read(struct file* file, void* buf, uint32_t count) 
{
    ...
    kassert(block_read_start_idx < 139 && block_read_end_idx < 139);
    ...
    {       
        /* 在同一扇区内读数据,不涉及到跨扇区读取 */
        kassert(block_read_end_idx == block_read_start_idx);
