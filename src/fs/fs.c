...
static bool mount_partition(struct node *pelem, int arg)
{
    char * part_name = (char *)arg;
    struct partition *part = container_of(struct partition, part_tag, pelem);
    ...
    if (!strcmp(part->name, part_name))
    {
        ...
        /* 1.将硬盘上的块位图读入到内存 */
        cur_part->block_bm.bits = (uint8_t *)
            sys_malloc(sb_buf->block_bitmap_sects * SECTOR_SIZE);
        if (cur_part->block_bm.bits == NULL)
        {
            PANIC("alloc memory failed!\n");
        }
        cur_part->block_bm.len = sb_buf->block_bitmap_sects * SECTOR_SIZE;

        /* 从硬盘上读入块位图到分区的block_bitmap.bits */
        ide_read(hd, sb_buf->block_bitmap_lba, cur_part->block_bm.bits,
                    sb_buf->block_bitmap_sects);

        /* 2.将硬盘上的inode位图读入到内存 */
        cur_part->inode_bm.bits = (uint8_t *)
            sys_malloc(sb_buf->inode_bitmap_sects * SECTOR_SIZE);
        if (cur_part->inode_bm.bits == NULL)
        {
            PANIC("alloc memory failed!\n");
        }
        cur_part->inode_bm.len = sb_buf->inode_bitmap_sects * SECTOR_SIZE;

        /* 从硬盘上读入inode位图到分区的inode_bitmap.bits */
        ide_read(hd, sb_buf->inode_bitmap_lba, cur_part->inode_bm.bits,
         ...
         * 与函数本身功能无关。只有返回true时list_traversal才会停止遍历，
         * 减少了后面元素无意义的遍历
         */
        return true;
    }
...
static void partition_format(struct partition * part)
{
    ...
    buf_size = (buf_size >= sb.inode_table_sects ? buf_size :
            sb.inode_table_sects) * SECTOR_SIZE;
                    ...
                    struct path_search_record * searched_record) 
{
    ...
    kassert(pathname[0] == '/' && path_len > 1 && path_len < MAX_PATH_LEN);
    ...
    {    
        ...
        kassert(strlen(searched_record->searched_path) < 512);
...
static uint32_t fd_local2global(uint32_t local_fd) 
{
    ...
    kassert(global_fd >= 0 && global_fd < MAX_FILE_OPEN);
...
{
    kassert(strlen(pathname) < MAX_PATH_LEN);
    ...
    kassert(file_idx == MAX_FILE_OPEN);
...
rollback:	     /* 因为某步骤操作失败而回滚 */
    switch (rollback_step) 
    {
        case 2:
            ...
            bitmap_set(&cur_part->inode_bm, inode_no, 0);	 
...
{
    kassert(strlen(name) < MAX_PATH_LEN);
...
int32_t sys_rmdir(const char* pathname) 
{
    ...
    else 
    {
        ...
        { 
            ...
            {    
                /* 非空目录不可删除 */
                printk("dir %s is not empty, it is not allowed to "
                        ...
                        void* io_buf) 
{
    ...
    kassert(block_lba >= cur_part->sb->data_start_lba);
    ...
    /* 第0个目录项是"."，第1个目录项是".." */
    kassert(dir_e[1].i_no < 4096 && dir_e[1].f_type == FT_DIRECTORY);
...
static int get_child_dir_name(uint32_t p_inode_nr, 
                    uint32_t c_inode_nr, char* path, void* io_buf) 
{
...
char* sys_getcwd(char* buf, uint32_t size) 
{
    ...
    /* 最大支持4096个inode */
    kassert(child_inode_nr >= 0 && child_inode_nr < 4096);   
    ...
    kassert(strlen(full_path_reverse) <= size);
...
void filesys_init(void)
{
    ...
    /* sb_buf用来存储从硬盘上读入的超级块 */
    struct super_block * sb_buf;
    sb_buf = (struct super_block *)sys_malloc(SECTOR_SIZE);
        ...
        while (dev_no < 2)
        {
            ...
            while (part_idx < 12)
            {
                ...
                if (part->sec_cnt != 0)     /* 如果分区存在 */
                {
                    ...
                    /* 读出分区的超级块，根据魔数是否正确来判断
                     *是否存在文件系统
                     */
                    ide_read(hd, part->start_lba + 1, sb_buf, 1);

                    /* 只支持自己的文件系统，若磁盘上已经有文件系统
                     * 就不再格式化
                     */
                    if (sb_buf->magic == 0x20170629)
                    {
