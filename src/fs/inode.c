...
{     
    ...
    kassert(inode_pos.sec_lba <= (part->start_lba + part->sec_cnt));
...
struct inode * inode_open(struct partition *part, uint32_t inode_no) 
{
     ...
     * 此链表是为提速而在内存中创建的缓冲区 
     */
    struct node * elem = part->open_inodes.head.next;
    struct inode * inode_found;
    while (elem != &part->open_inodes.tail) 
    {
        inode_found = container_of(struct inode, inode_tag, elem);
...
/* 将硬盘分区part上的inode清空 
 * 即把inode从inode_table中擦除，把该inode对应的空间清0
 */
...
void inode_release(struct partition* part, uint32_t inode_no) 
{
    ...
    kassert(inode_to_del->i_no == inode_no);
    ...
    /* 2.回收该inode所占用的inode位图 */
    bitmap_set(&part->inode_bm, inode_no, 0);  
