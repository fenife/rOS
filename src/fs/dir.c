...
{
    kassert(strlen(filename) <=  MAX_FILE_NAME_LEN);
                            ...
                            void * io_buf) 
{
    ...
    kassert(dir_size % dir_entry_size == 0);   
    ...
    {  
        ...
        {   
            ...
            {    
                ...
                if (block_lba == -1) 
                {
                    ...
                    bitmap_set(&cur_part->block_bm, block_bitmap_idx, 0);
        ...
        {
            /* FT_UNKNOWN为0,无论是初始化或是删除文件后，
             * 都会将f_type置为FT_UNKNOWN 
             */
            if ((dir_e + dir_entry_idx)->f_type == FT_UNKNOWN) 
            {   
                memcpy(dir_e + dir_entry_idx, p_de, dir_entry_size);    
                ide_write(cur_part->my_disk, all_blocks[block_idx], 
                            io_buf, 1);
                        ...
                        uint32_t inode_no, void* io_buf) 
{
    ...
    while (block_idx < 140) 
    {
        ...
        kassert(dir_entry_cnt >= 1);
        
        /* 除目录第1个扇区外，若该扇区上只有该目录项自己，
         * 则将整个扇区回收 
         */
        if (dir_entry_cnt == 1 && !is_dir_first_block) 
        {
            ...
            bitmap_set(&part->block_bm, block_bitmap_idx, 0);
            bitmap_sync(cur_part, block_bitmap_idx, BLOCK_BITMAP);
            ...
            {   
                ...
                kassert(indirect_blocks >= 1);  /* 包括当前间接块 */
        ...
        /* 更新i结点信息并同步到硬盘 */
        kassert(dir_inode->i_size >= dir_entry_size);
        dir_inode->i_size -= dir_entry_size;
    ...
    while (block_idx < block_cnt)
    {
        ...
        while (dir_entry_idx < dir_entrys_per_sec) 
        {
            ...
            {  
                ...
                kassert(cur_dir_entry_pos == dir->dir_pos);
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir)
{
    ...
    while (block_idx < 13) 
    {
        kassert(child_dir_inode->i_sectors[block_idx] == 0);
        block_idx++;
    }
    ...
    /* 回收inode中i_secotrs中所占用的扇区，
     * 并同步inode_bitmap和block_bitmap 
     */
